...
"""))
  ...
  def _write_dex_fc_properties(self, env):
    ...
    fp.write(env.expandArgs("""
#Autogenerated by install script
database.uri=postgresql://$DBUSER:$DBPWD@$DBHOST/$DBNAME?pool_max_size=$BDB_POOL_MAX_SIZE
...
"""))
  ...
  def _copy_dex_user_properties(self, env):
    # XXX: this will work only for our tomcat deployment 
    source = env.expandArgs("$TPREFIX/tomcat/webapps/BaltradDex/WEB-INF/conf/dex.user.properties")
    target = "./WEB-INF/conf/dex.user.properties"
