...
"""))
  ...
  def _write_log4j_properties(self, env):
...
log4j.logger.BEAST=INFO, dexappender
log4j.additivity.DEX=false
log4j.additivity.PGF=false
log4j.additivity.BEAST=false
log4j.appender.dexappender=eu.baltrad.dex.log.model.LogAppender
...
"""))
  ...
  def _copy_dex_config_property_file(self, env, configfile):
    ...
    oldvariant = env.expandArgs("$TPREFIX/tomcat/webapps/BaltradDex/WEB-INF/conf/%s"%configfile)
    if os.path.exists(oldvariant):
      shutil.move(oldvariant, env.expandArgs("$TPREFIX/tomcat/webapps/BaltradDex/%s"%configfile))
    source1 = env.expandArgs("$TPREFIX/tomcat/webapps/BaltradDex/%s"%configfile)
    source2 = "%s"%configfile
    lines = []
