...
class nodescripts(object):
  ...
  def create_scripts(self, env):
    ...
    self._raveloggerinitdscript = self._create_rave_logger_initd_script(env)
...
check_ravepgf_logger_status() {
  RAVEPGFPROCESS=`$PREFIX/rave/bin/rave_pgf_logger -a status`
  if [ "$$RAVEPGFPROCESS" = "$PREFIX/rave/bin/rave_pgf_logger is not running" ]; then
    return 1
  fi
  return 0
}

start_ravepgf_logger() {
  echo -n "Starting rave-pgf logger..."
  check_ravepgf_logger_status
  if [ $$? -eq 0 ]; then
    echo " already running..."
  else
    "$PREFIX/rave/bin/rave_pgf_logger" -a start
    echo ""
  fi
}

stop_ravepgf_logger() {
  echo "Stopping rave-pgf logger..."
  "$PREFIX/rave/bin/rave_pgf_logger" -a stop
}

status_ravepgf_logger() {
  check_ravepgf_logger_status
  if [ $$? -eq 0 ]; then
    echo "Running"
  else
...
print_usage() {
  ...
  echo "         --ravepgf-logger - only affect ravepgf logger"
...
if [ "$${ACTIVATE_RAVE}" = "no" -a "$${RAVEPGF_LOGGER_REQUEST}" = "yes" ]; then
  echo "RavePGF support not enabled!"
  exit 1;
fi
...
if [ "$${ACTIVATE_NODE}" = "no" -a "$${RAVEPGF_REQUEST}" = "no" -a "$${RAVEPGF_LOGGER_REQUEST}" = "no" -a  "$${BDB_REQUEST}" = "no" -a "$${ALL_REQUEST}" = "no" ]; then
...
if [ "$${START_REQUEST}" = "yes" ]; then
  if [ "$${RAVEPGF_REQUEST}" = "yes" ]; then
    start_ravepgf
  elif [ "$${RAVEPGF_LOGGER_REQUEST}" = "yes" ]; then
    start_ravepgf_logger
...
"""))
  ...
  # @param env: the build environment
  #
  def _create_rave_logger_initd_script(self, env):
    fpd, filename = tempfile.mkstemp(suffix=".tmp", prefix="ravepgf_logger.init.d")
    ofp = os.fdopen(fpd, "w")
    ofp.write(env.expandArgs("""
#!/bin/sh
# Autogenerated by baltrad-core/setup
...
start() {
  su - $$NODEUSER -c "$PREFIX/bin/bltnode --ravepgf-logger start"
}

stop() {
  su - $$NODEUSER -c "$PREFIX/bin/bltnode --ravepgf-logger stop"
}
...
"""))
  ...
  ##
  # @return: the temporary rave logger init.d script file name
  #
  def get_rave_logger_initd_script_path(self):
    return self._raveloggerinitdscript
...
""", extras))
  ...
  ##
  # Uses the script to identify if the rave pgf logger is running
  # @return: True if it is running, otherwise False
  # 
  def _isRaveLoggerRunning(self):
    status = subprocess.Popen("%s --ravepgf-logger status"%self._nodescript, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,shell=True).communicate()[0]
    print "STATUS: %s"%status
    if status.find("Rave PGF Logger: Running") >= 0:
      return True
    return False 
  ...
  ##
  # Waits for the rave pgf logger to stop. A stop request should have been issued
  # prior to this method is called.
  # @param msg: A message to show that it is waiting
  # @param sec: how many seconds to wait
  # 
  # @return: True if the rave pgf logger has been stopped, otherwise False
  #  
  def _waitForRaveLoggerStop(self, msg, sec):
    import time, sys
    
    print msg
    
    for i in range(sec):
      if self._isRaveLoggerRunning():
        print ".",
        sys.stdout.flush()
        time.sleep(1)
      else:
  ...
  def start(self, node=False, rave=False):
    ...
    if rave and self._raveinstalled and not self._isRaveLoggerRunning():
      ocode = subprocess.call("%s --ravepgf-logger start"%(self._nodescript), shell=True)
      if ocode != 0:
        raise Exception, "Failed to start rave logger"
  ...
  def stop(self, node=False, rave=False):
    ...
    if rave and self._raveinstalled and self._isRaveLoggerRunning():
      ocode = subprocess.call("%s --ravepgf-logger stop"%(self._nodescript),shell=True)
      if ocode != 0:
        raise Exception, "Failed to stop rave logger"
    
      if not self._waitForRaveLoggerStop("Waiting for rave logger to stop", 10):
        raise Exception, "Failed to stop rave logger"
